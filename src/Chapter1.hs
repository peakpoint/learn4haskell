...
mid :: Int -> Int -> Int -> Int
mid x y z
    | x < y = if z < y then max x z else y
    | otherwise = if z < x then max y z else x
...
sumLast2 :: Int -> Int
sumLast2 n = 
    let mod10 = mod (abs n) 10
        mod100 = mod (abs n) 100
    in div (mod100 - mod10) 10 + mod10
...
firstDigit n
    | m < 10 = m
    | otherwise = firstDigit (div (m - mod m 10) 10)
    where m = abs n -- this really only needs to be applied once at the start
